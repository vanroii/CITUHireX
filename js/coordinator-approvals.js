...
if (auth) {
  ...
  let expandedAppId = null // which application's "View Details" panel is open
  let expandedAppJobId = null // which application's nested "View Job" panel is open
  ...
  async function loadJobs() {
    ...
    el.querySelectorAll('.decline-job-btn').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation()
        btn.disabled = true
        btn.textContent = 'Declining…'
        await supabase.from('job_postings').update({ status: 'declined' }).eq('id', btn.dataset.jobId)
        expandedJobId = null
        loadJobs()
      })
    })
    ...
    <div class="row-card job-approval-card" style="flex-direction:column; align-items:stretch;" data-job-id="${job.id}">
      ...
      <div class="job-approval-details" style="margin-top:16px; padding-top:16px; border-top:1px solid var(--gray-200);">
        ...
        <div class="row-actions" style="margin-top:16px;">
          ...
          <button class="btn btn-ghost btn-sm decline-job-btn" data-job-id="${job.id}">Decline</button>
  ...
  async function loadApps() {
    const { data: apps } = await supabase
      .from('applications')
      .select(
        '*, students(profile_id, programs(name), profiles(full_name)), job_postings(title, description, location, required_skills, is_remote, company_id, companies(company_name))'
      )
    ...
    el.querySelectorAll('.app-approval-header').forEach((header) => {
      header.addEventListener('click', () => {
        const id = header.closest('.app-approval-card').dataset.appId
        expandedAppId = expandedAppId === id ? null : id
        loadApps()
      })
    })

    el.querySelectorAll('.app-approval-details').forEach((d) => d.addEventListener('click', (e) => e.stopPropagation()))
  ...
  function renderAppCard(app) {
    ...
    return `
      <div class="row-card app-approval-card" style="flex-direction:column; align-items:stretch;" data-app-id="${app.id}">
        <div class="app-approval-header" style="cursor:pointer; display:flex; justify-content:space-between; align-items:center; gap:16px; flex-wrap:wrap;">
          <div>
        ...
        ${expanded ? renderAppDetails(app, job, showJob) : ''}
      </div>`
  }

  function renderAppDetails(app, job, showJob) {
    return `
      <div class="app-approval-details" style="margin-top:16px; padding-top:16px; border-top:1px solid var(--gray-200);">
        <button class="btn btn-ghost btn-sm view-job-btn" data-app-id="${app.id}" style="margin-bottom:12px;">${showJob ? 'Hide Job ▲' : 'View Job ▼'}</button>
        ${
          showJob
            ? `
        <div style="margin-bottom:16px; padding:16px; background:var(--gray-100); border-radius:10px;">
          ...
          <p class="sub-meta">${job.location || ''}${job.is_remote ? ' · Remote' : ''}</p>
        </div>`
            : ''
        }
